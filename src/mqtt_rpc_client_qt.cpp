...
MqttRpcClientQt::WebSocketIODevice::WebSocketIODevice(QObject *parent)
	: QIODevice(parent)
{
...
bool MqttRpcClientQt::WebSocketIODevice::isSequential() const
{
	return true;
}

qint64 MqttRpcClientQt::WebSocketIODevice::bytesAvailable() const
{
	return static_cast<qint64>(m_buffer.size()) + QIODevice::bytesAvailable();
}

bool MqttRpcClientQt::WebSocketIODevice::open(QIODevice::OpenMode mode)
{
...
void MqttRpcClientQt::WebSocketIODevice::close()
{
	m_socket.close();
	QIODevice::close();
}

qint64 MqttRpcClientQt::WebSocketIODevice::readData(char *data, qint64 maxlen)
{
...
qint64 MqttRpcClientQt::WebSocketIODevice::writeData(const char *data, qint64 len)
{
...
void MqttRpcClientQt::WebSocketIODevice::setUrl(const QUrl &url)
{
	m_url = url;
}

void MqttRpcClientQt::WebSocketIODevice::setProtocol(const QByteArray &data)
{
	m_protocol = data;
}
...
void MqttRpcClientQt::WebSocketIODevice::handleBinaryMessage(const QByteArray &msg)
{
	m_buffer.append(msg);
	emit readyRead();
}

void MqttRpcClientQt::WebSocketIODevice::onSocketConnected()
{
...
void MqttRpcClientQt::init_mqtt_client()
{
	if(!username.isEmpty() && !password.isEmpty()) {
		mqtt_client.setHostname(host_name.isEmpty() ? MQTT_GLOBAL_BROKER_HOST : host_name);
		mqtt_client.setPort(port == 0 ? MQTT_GLOBAL_BROKER_PORT : port);

		mqtt_client.setUsername(username);
		mqtt_client.setPassword(password.toUtf8());
	} else {
		mqtt_client.setHostname(host.toString());
...
void MqttRpcClientQt::connectClientSignals(){
	...
	connect(&mqtt_client, &QMqttClient::connected, this, &MqttRpcClientQt::on_connect);
	connect(&mqtt_client, &QMqttClient::messageReceived, this, &MqttRpcClientQt::on_message);
	connect(&mqtt_client, &QMqttClient::errorChanged, this, &MqttRpcClientQt::on_error);
	connect(&mqtt_client, &QMqttClient::pingResponseReceived, this, &MqttRpcClientQt::pingresp);
	if (web_socket_device) {
		connect(web_socket_device, &WebSocketIODevice::socketConnected, this, [this]() {
			qDebug() << MQTT_RPC_CLIENT_LOGGING_PREFIX << "WebSocket connected";

			mqtt_client.setTransport(web_socket_device, QMqttClient::IODevice);
			mqtt_client.setKeepAlive(30);
			mqtt_client.connectToHost();
		});

		if (!web_socket_device->open(QIODevice::ReadWrite)) {
			qWarning() << MQTT_RPC_CLIENT_LOGGING_PREFIX << "Could not open websocket device";
		}
		...
		mqtt_client.connectToHostEncrypted(sslConfig);
	} else {
		mqtt_client.connectToHost();
	}
}
...
void MqttRpcClientQt::on_error(const QMqttClient::ClientError error) {
...
void MqttRpcClientQt::on_message(const QByteArray &message, const QMqttTopicName &topic) {
	// https://erickveil.github.io/2016/04/06/How-To-Manipulate-JSON-With-C++-and-Qt.html
	qDebug().noquote() << MQTT_RPC_CLIENT_LOGGING_PREFIX << "got message: " << message;
	...
	locker.unlock();
	if(command) {
		qInfo().noquote() << QString("receive < %1 : %3").arg(topic.name()).arg(QString::fromUtf8(message));
